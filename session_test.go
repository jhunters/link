...
func NewTestCodec(rw io.ReadWriter) (Codec[Pojo, Pojo], error) {
	return &TestCodec[Pojo]{
		rw: rw.(io.ReadWriteCloser),
	}, nil
}

type TestCodec[E Pojo] struct {
	rw io.ReadWriteCloser
}

func (c *TestCodec[E]) Send(msg Pojo) error {

	data := make([]byte, 8)
	binary.LittleEndian.PutUint64(data, uint64(msg.Age))
...
func (c *TestCodec[E]) Receive() (Pojo, error) {
	var head [8]byte
	_, err := io.ReadFull(c.rw, head[:])
	if err != nil {
		return Pojo{}, err
	}
	age := binary.LittleEndian.Uint64(head[:])
	if err != nil {
		return Pojo{}, err
	}
	return Pojo{Age: age}, nil
}

func (c *TestCodec[E]) Close() error {
	return c.rw.Close()
}

func (c *TestCodec[E]) ClearSendChan(ch <-chan interface{}) {
	for _ = range ch {
	}
}
